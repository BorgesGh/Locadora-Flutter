...
class GenericService<T> {
  ...
  final T Function(Map<String,dynamic> map) fromMap;// Adiciona uma função padrão para criar novos atores
  ...
  Future<ResponseEntity<List<T>>> getAll() async{
    ...
    try {
      ...
      if (_response.statusCode == 200) {
        ...
        final listAtores = listMapResponse.map<T>((atorJson) =>
            fromMap(atorJson)).toList();
  ...
  Future<ResponseEntity<T>> inserir(T elemento) async {
    ...
    try {
      ...
      var _response = await client.post(
        _uri,
        body: jsonEncode((elemento as dynamic).toMap()), // Esse cast dá acesso ao metodo toMap
